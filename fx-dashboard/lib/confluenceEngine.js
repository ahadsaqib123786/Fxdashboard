...
function checkTopDownAlignment(dailyBias, htfBias, mtfBias, ltfDirection) {
  ...
  if (ltfDirection && ltfDirection !== mtfBias) {
    return { aligned: false, reason: `1H bias (${mtfBias.toUpperCase()}) disagrees with the latest 30M structure break (${ltfDirection.toUpperCase()}) — rejected before scoring.` };
  }
...
export async function buildConfluence(symbol, options = {}) {
  ...
  const dailyDirection = directionFromBias(dailyScore.bias);

  if (!dailyDirection) {
    return finalizeNoBias(symbol, currentPrice, dailyScore, "No Daily directional bias — nothing to align 4H, 1H or 30M against.");
  }

  const htfStructure = detectStructure(htf);
  const mtfStructure = detectStructure(mtf);
  const ltfStructure = detectStructure(ltf);
  const ltfDirection = lastStructureDirection(ltfStructure);

  const alignment = checkTopDownAlignment(dailyScore.bias, htfScore.bias, mtfScore.bias, ltfDirection);
  if (!alignment.aligned) {
    return finalizeRejected(symbol, currentPrice, { daily: dailyScore, htf: htfScore, mtf: mtfScore }, alignment.reason);
  }
  ...
  const htfObs = markMitigation(detectOrderBlocks(htf), htf);
  const mtfFvgs = markMitigation(detectFVGs(mtf), mtf);
  const mtfObs = markMitigation(detectOrderBlocks(mtf), mtf);
  const ltfFvgs = markMitigation(detectFVGs(ltf), ltf);
  const ltfObs = markMitigation(detectOrderBlocks(ltf), ltf);
  ...
  return {
    ...
    invalidation,
    reachability,
    daily: { timeframe: "Daily", bias: dailyScore.bias, reason: dailyScore.reason },
...
function stripPrices({ type, time, mitigated }) {
  return { type, time, mitigated };
}
...
function finalizeNoBias(symbol, currentPrice, dailyScore, reason) {
  ...
  return {
    ...
    invalidation: [],
    daily: { timeframe: "Daily", bias: "neutral", reason: dailyScore.reason },
...
function finalizeRejected(symbol, currentPrice, scores, reason) {
  const breakdown = Object.fromEntries(
    Object.entries(CONFLUENCE_WEIGHTS).map(([key, max]) => [key, { points: 0, max, note: "Not scored — rejected at the top-down alignment check." }])
  );
  return {
    symbol,
    currentPrice,
    ...
    expectedBehaviour: null,
    invalidation: [],
    daily: { timeframe: "Daily", bias: scores.daily.bias, reason: scores.daily.reason },
    htf: { timeframe: "4H", bias: scores.htf.bias, reason: scores.htf.reason },
    mtf: { timeframe: "1H", bias: scores.mtf.bias, reason: scores.mtf.reason },
    ltf: { timeframe: "30M", fairValueGaps: [], orderBlocks: [] },
    session: currentSessionLabel(),
    timeframesAligned: false,
  };
