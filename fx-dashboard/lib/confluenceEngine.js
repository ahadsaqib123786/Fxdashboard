...
function buildTradePlan({ direction, currentPrice, zone, poiKind, alternativesCount, ltfCandles }) {
  if (!zone) return null;
  ...
  return {
    ...
    riskReward: "1:2 / 1:3",
    poi: {
      kind: poiKind, // "Order Block" | "Fair Value Gap"
      time: zone.time,
      alternativesConsidered: alternativesCount, // how many other unmitigated zones in this direction existed
    },
  };
}
...
export async function buildConfluence(symbol, options = {}) {
  ...
  const poiKind = relevantOBs[0] ? "Order Block" : relevantFVGs[0] ? "Fair Value Gap" : null;
  const alternativesCount = Math.max(relevantOBs.length + relevantFVGs.length - 1, 0);
  const tradePlan = qualifies
    ? buildTradePlan({ direction, currentPrice, zone: bestZone, poiKind, alternativesCount, ltfCandles: ltf })
    : null;
