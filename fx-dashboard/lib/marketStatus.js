...
export function deriveMarketStatus({ bestTrade, ranked }, now = new Date()) {
  ...
  const imminentNews = ranked.some((r) =>
    r.breakdown?.newsFilter && r.breakdown.newsFilter.points === 0 && r.direction
  );

  if (bestTrade) {
    const status = bestTrade.grade === "A+" ? STATUS.A_PLUS : STATUS.GOOD;
    return {
      status,
      headline:
        bestTrade.grade === "A+"
          ? `${bestTrade.symbol} is an A+ setup — ${bestTrade.totalScore}/${bestTrade.maxScore} confidence.`
          : `${bestTrade.symbol} qualifies as a ${bestTrade.grade}-grade opportunity — ${bestTrade.totalScore}/${bestTrade.maxScore} confidence.`,
    };
  }
  ...
  const anyWatchable = ranked.some((r) => r.grade === "Ignore" && r.totalScore >= 50);
  if (anyWatchable) {
    return {
      status: STATUS.WATCHLIST,
      headline: "No A+ setups currently available. A few pairs are building — worth watching, not trading yet.",
    };
  }
